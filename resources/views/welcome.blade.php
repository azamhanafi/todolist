...
<body class="bg-gray-200 p-4">
    ...
    <script>
        window.addEventListener("visibilitychange", function () {
            if (document.visibilityState === "hidden") {
                localStorage.clear();
                console.log("Session reset: To-Do list cleared.");

                // Optional: Send an AJAX request to the server to clear database data
                fetch('/', { method: 'DELETE' }, headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                })
                    .then(response => {
                        if (!response.ok) {
                            console.error('Error clearing data from database:', response.statusText);
                        } else {
    ...
    <div class="lg:w-2/4 mx-auto py-8 px-6 bg-white rounded-xl">
        ...
        <div class="mb-6">
            <form class="flex flex-col space-y-4" method="POST" action="/todolist/todolist/public/">
                @csrf
            ...
            @foreach ($todos as $todo)
                ...
                <div class="py-4 flex items-center border-b border-gray-300 px-3 {{$todo->isDone ? 'bg-green-200' : ''}}">
                    ...
                    <div class="flex space-x-3">
                        <form method="POST" action="/todolist/todolist/public/{{$todo->id}}">
                            @csrf
                            @method('PATCH')
